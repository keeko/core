...
abstract class BaseAppUriQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'keeko', $modelName = 'keeko\\core\\entities\\AppUri', $modelAlias = null)
    {
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(AppUriPeer::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(AppUriPeer::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     * $query->filterByAppId(array('max' => 12)); // WHERE app_id <= 12
     * </code>
     *
     * @see       filterByApp()
     *
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\AppQuery A secondary query class using the current class as primary query
     */
    public function useAppQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinApp($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'App', '\keeko\core\entities\AppQuery');
    }
