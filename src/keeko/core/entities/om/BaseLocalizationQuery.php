 ...
 *
 * @method LocalizationQuery leftJoinApplicationUri($relationAlias = null) Adds a LEFT JOIN clause to the query using the ApplicationUri relation
 * @method LocalizationQuery rightJoinApplicationUri($relationAlias = null) Adds a RIGHT JOIN clause to the query using the ApplicationUri relation
 * @method LocalizationQuery innerJoinApplicationUri($relationAlias = null) Adds a INNER JOIN clause to the query using the ApplicationUri relation
 *
...
abstract class BaseLocalizationQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'keeko', $modelName = 'keeko\\core\\entities\\Localization', $modelAlias = null)
    {
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(LocalizationPeer::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(LocalizationPeer::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     * $query->filterByCountryIsoNr(array('max' => 12)); // WHERE country_iso_nr <= 12
     * </code>
     *
     * @see       filterByCountry()
     *
    ...
    public function useLocalizationRelatedByParentIdQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'LocalizationRelatedByParentId', '\keeko\core\entities\LocalizationQuery');
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\CountryQuery A secondary query class using the current class as primary query
     */
    public function useCountryQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinCountry($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Country', '\keeko\core\entities\CountryQuery');
    }
    ...
    public function useLocalizationRelatedByIdQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'LocalizationRelatedById', '\keeko\core\entities\LocalizationQuery');
    }
    ...
    public function filterByApplicationUri($applicationUri, $comparison = null)
    {
        if ($applicationUri instanceof ApplicationUri) {
            return $this
                ->addUsingAlias(LocalizationPeer::ID, $applicationUri->getLocalizationId(), $comparison);
