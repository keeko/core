...
use keeko\core\entities\CurrencyQuery;
...
abstract class BaseCurrencyQuery extends ModelCriteria
{
    ...
    public function filterByIsoNr($isoNr = null, $comparison = null)
    {
        if (is_array($isoNr)) {
            $useMinMax = false;
            if (isset($isoNr['min'])) {
                $this->addUsingAlias(CurrencyPeer::ISO_NR, $isoNr['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($isoNr['max'])) {
                $this->addUsingAlias(CurrencyPeer::ISO_NR, $isoNr['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     * $query->filterByDecimalDigits(array('max' => 12)); // WHERE decimal_digits <= 12
     * </code>
     *
     ...
     * $query->filterBySubDivisor(array('max' => 12)); // WHERE sub_divisor <= 12
     * </code>
     *
