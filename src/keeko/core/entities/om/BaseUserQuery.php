...
abstract class BaseUserQuery extends ModelCriteria
{
     ...
     * $query->filterBySubdivisionId(array('max' => 12)); // WHERE subdivision_id <= 12
     * </code>
     *
     ...
     * $query->filterByLocationStatus(array('max' => 12)); // WHERE location_status <= 12
     * </code>
     *
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\GroupQuery A secondary query class using the current class as primary query
     */
    public function useGroupQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinGroup($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Group', '\keeko\core\entities\GroupQuery');
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\GroupUserQuery A secondary query class using the current class as primary query
     */
    public function useGroupUserQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinGroupUser($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'GroupUser', '\keeko\core\entities\GroupUserQuery');
    }
