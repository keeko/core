...
use keeko\core\entities\SubdivisionPeer;
use keeko\core\entities\SubdivisionQuery;
...
abstract class BaseSubdivisionQuery extends ModelCriteria
{
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(SubdivisionPeer::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(SubdivisionPeer::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     * $query->filterBySubdivisionTypeId(array('max' => 12)); // WHERE subdivision_type_id <= 12
     * </code>
     *
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\SubdivisionTypeQuery A secondary query class using the current class as primary query
     */
    public function useSubdivisionTypeQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'SubdivisionType', '\keeko\core\entities\SubdivisionTypeQuery');
    }
