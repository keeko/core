...
/**
 * Base class that represents a row from the 'keeko_block' table.
 *
 *
 *
...
abstract class BaseBlock extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'keeko\\core\\entities\\BlockPeer';
    ...
    public function setLayoutId($v)
    {
        ...
        if ($this->layout_id !== $v) {
            $this->layout_id = $v;
            $this->modifiedColumns[] = BlockPeer::LAYOUT_ID;
        }
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(BlockPeer::LAYOUT_ID)) {
            $modifiedColumns[':p' . $index++]  = '`layout_id`';
        }

        $sql = sprintf(
            'INSERT INTO `keeko_block` (%s) VALUES (%s)',
