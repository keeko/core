...
use keeko\core\entities\Subdivision;
...
abstract class BaseCountryQuery extends ModelCriteria
{
     ...
     * $query->filterByIsoNr(array('max' => 12)); // WHERE iso_nr <= 12
     * </code>
     *
    ...
    public function filterByIsoNr($isoNr = null, $comparison = null)
    {
        if (is_array($isoNr)) {
            $useMinMax = false;
            if (isset($isoNr['min'])) {
                $this->addUsingAlias(CountryPeer::ISO_NR, $isoNr['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($isoNr['max'])) {
                $this->addUsingAlias(CountryPeer::ISO_NR, $isoNr['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     * $query->filterByTerritoryIsoNr(array('max' => 12)); // WHERE territory_iso_nr <= 12
     * </code>
     *
     ...
     * $query->filterByCurrencyIsoNr(array('max' => 12)); // WHERE currency_iso_nr <= 12
     * </code>
     *
     ...
     * $query->filterByBboxSwLat(array('max' => 12)); // WHERE bbox_sw_lat <= 12
     * </code>
     *
     ...
     * $query->filterByBboxSwLng(array('max' => 12)); // WHERE bbox_sw_lng <= 12
     * </code>
     *
     ...
     * $query->filterByBboxNeLat(array('max' => 12)); // WHERE bbox_ne_lat <= 12
     * </code>
     *
     ...
     * $query->filterByBboxNeLng(array('max' => 12)); // WHERE bbox_ne_lng <= 12
     * </code>
     *
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\TerritoryQuery A secondary query class using the current class as primary query
     */
    public function useTerritoryQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinTerritory($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Territory', '\keeko\core\entities\TerritoryQuery');
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\CurrencyQuery A secondary query class using the current class as primary query
     */
    public function useCurrencyQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCurrency($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Currency', '\keeko\core\entities\CurrencyQuery');
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\UserQuery A secondary query class using the current class as primary query
     */
    public function useUserQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUser($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'User', '\keeko\core\entities\UserQuery');
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \keeko\core\entities\SubdivisionQuery A secondary query class using the current class as primary query
     */
    public function useSubdivisionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'Subdivision', '\keeko\core\entities\SubdivisionQuery');
    }
