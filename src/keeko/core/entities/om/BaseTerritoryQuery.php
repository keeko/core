...
use keeko\core\entities\TerritoryQuery;
...
abstract class BaseTerritoryQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'keeko', $modelName = 'keeko\\core\\entities\\Territory', $modelAlias = null)
    {
    ...
    public function filterByIsoNr($isoNr = null, $comparison = null)
    {
        if (is_array($isoNr)) {
            $useMinMax = false;
            if (isset($isoNr['min'])) {
                $this->addUsingAlias(TerritoryPeer::ISO_NR, $isoNr['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($isoNr['max'])) {
                $this->addUsingAlias(TerritoryPeer::ISO_NR, $isoNr['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     * $query->filterByParentIsoNr(array('max' => 12)); // WHERE parent_iso_nr <= 12
     * </code>
     *
