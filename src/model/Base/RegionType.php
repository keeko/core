...
abstract class RegionType implements ActiveRecordInterface
{
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildRegionTypeQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see RegionType::setDeleted()
     * @see RegionType::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildRegionTypeQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                RegionTypeTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aRegionArea !== null) {
                if ($this->aRegionArea->isModified() || $this->aRegionArea->isNew()) {
                    $affectedRows += $this->aRegionArea->save($con);
                }
                $this->setRegionArea($this->aRegionArea);
            }
            ...
            if ($this->countriesRelatedByTypeIdScheduledForDeletion !== null) {
                if (!$this->countriesRelatedByTypeIdScheduledForDeletion->isEmpty()) {
                    \keeko\core\model\CountryQuery::create()
                        ->filterByPrimaryKeys($this->countriesRelatedByTypeIdScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->countriesRelatedByTypeIdScheduledForDeletion = null;
                }
            }

            if ($this->collCountriesRelatedByTypeId !== null) {
                foreach ($this->collCountriesRelatedByTypeId as $referrerFK) {
            ...
            if ($this->countriesRelatedBySubtypeIdScheduledForDeletion !== null) {
                if (!$this->countriesRelatedBySubtypeIdScheduledForDeletion->isEmpty()) {
                    foreach ($this->countriesRelatedBySubtypeIdScheduledForDeletion as $countryRelatedBySubtypeId) {
                        // need to save related object because we set the relation to null
                        $countryRelatedBySubtypeId->save($con);
                    }
                    $this->countriesRelatedBySubtypeIdScheduledForDeletion = null;
                }
            }

            if ($this->collCountriesRelatedBySubtypeId !== null) {
                foreach ($this->collCountriesRelatedBySubtypeId as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $sql = sprintf(
            'INSERT INTO `kk_region_type` (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\keeko\core\model\RegionType
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\keeko\core\model\RegionType The current object, for fluid interface
     */
